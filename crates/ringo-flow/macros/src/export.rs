...
pub(crate) fn ts_export(attr: &TokenStream2, mut imp: ItemImpl) -> TokenStream2 {
    ...
    quote! {
        ...
        #submits
        ::inventory::submit! {
            #model::TsInterfaceDef { name: #group, generic: #generic }
        }
    }
}
...
pub(crate) fn ts_global(attr: &TokenStream2, mut f: ItemFn) -> TokenStream2 {
    ...
    let submit = if jd.sigs.is_empty() {
        ...
        submit_tokens("global", &name, "Global", &params, &ret, &doc, &generic, false, 0, "")
    } else {
        ...
        for (i, s) in jd.sigs.iter().enumerate() {
            ...
            ts.extend(submit_tokens("global", &name, "Global", &[], "", &d, "", false, i as u32, s));
        }
        ts
    };
    strip_fn_attrs(&mut f);
...
fn method_decls(group: &str, sig: &syn::Signature, attrs: &[Attribute], order: &mut u32) -> Vec<TokenStream2> {
    ...
    let ret = if !is_getter && ret_opt { format!("{ret} | undefined") } else { ret };
    let o = *order;
    *order += 1;
    vec![submit_tokens(group, &name, kind, &params, &ret, &doc, "", optional, o, "")]
}
