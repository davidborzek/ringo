...
pub(crate) fn return_ts(sig: &syn::Signature) -> (String, bool) {
    ...
    if sig.asyncness.is_some() {
        ...
        let body = if optional { format!("{inner} | undefined") } else { inner };
...
pub(crate) fn map_type(ty: &Type) -> (String, bool) {
    ...
    for m in ["HashMap", "BTreeMap", "IndexMap"] {
        if let Some((k, v)) = generic_pair(ty, m) {
            return (format!("Record<{}, {}>", map_type(&k).0, map_type(&v).0), false);
        }
    }
...
fn generic_pair(ty: &Type, wrapper: &str) -> Option<(Type, Type)> {
    ...
    if let syn::PathArguments::AngleBracketed(ab) = &seg.arguments {
        let tys: Vec<Type> = ab
            .args
            .iter()
            .filter_map(|a| if let syn::GenericArgument::Type(t) = a { Some(t.clone()) } else { None })
            .collect();
        if tys.len() >= 2 {
