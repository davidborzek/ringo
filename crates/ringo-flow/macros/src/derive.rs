...
pub(crate) fn interface(input: DeriveInput) -> TokenStream2 {
    ...
    let name = util::jsdoc(&input.attrs).rename.unwrap_or_else(|| ident.to_string());
    ...
    let ro = if util::jsdoc(&input.attrs).readonly { "readonly " } else { "" };
...
pub(crate) fn ts_enum(input: DeriveInput) -> TokenStream2 {
    ...
    for v in &e.variants {
        ...
        let value = util::jsdoc(&v.attrs).value.unwrap_or_else(|| vname.to_lowercase());
