...
use super::dsl::agent::agent_global;
...
use super::dsl::mock::{
    json_response_global, mock_server_global, regex_global, text_response_global,
};
...
pub fn install(
    ...
    set!("scenario", |name: String, a: Value<'_>, b: Opt<Function<'_>>| -> rquickjs::Result<()> {
    ...
    set!("verifyAudioConnection", Async(verify_audio_connection_async));
    set!("regex", regex_global);
    set!("jsonResponse", json_response_global);
    set!("textResponse", text_response_global);
    set!("mockServer", mock_server_global);

    // Agents + assertions.
    set!("agent", agent_global);
...
pub(super) fn reject_unknown_keys(label: &str, config: &Object<'_>, allowed: &[&str]) -> Result<(), String> {
