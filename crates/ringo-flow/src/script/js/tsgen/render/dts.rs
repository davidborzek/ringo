...
pub fn dts(api: &Api, header: &str) -> String {
    ...
    for iface in &ifaces {
        let dup = api.types.iter().any(|t| {
            t.decl.trim_start().strip_prefix("interface ").and_then(|s| s.split([' ', '<', '{']).next())
                == Some(iface.name)
        });
        assert!(!dup, "interface `{}` declared by both #[ts_export] and a type declaration", iface.name);
    }
    for iface in ifaces {
        let _ = writeln!(out, "interface {}{} {{", iface.name, iface.generic);
        let mut members: Vec<&TsDecl> = api.decls.iter().copied().filter(|d| d.group == iface.name).collect();
        members.sort_by(|a, b| a.order.cmp(&b.order).then(a.raw.cmp(b.raw)).then(a.name.cmp(b.name)));
        for d in members {
    ...
    let mut globals: Vec<&TsDecl> = api.decls.iter().copied().filter(|d| d.group == "global").collect();
    globals.sort_by(|a, b| a.name.cmp(b.name).then(a.order.cmp(&b.order)).then(a.raw.cmp(b.raw)));
    for d in globals {
        render_doc(&mut out, d.doc, "");
        if d.raw.is_empty() {
            let _ = writeln!(out, "declare function {}{}({}): {};", d.name, d.generic, params_ts(d.params), d.ret);
        } else {
...
fn render_member(out: &mut String, d: &TsDecl, generic: &str) {
    ...
    if !d.raw.is_empty() {
        ...
        let _ = writeln!(out, "  readonly {}{}: {};", d.name, q, apply_generic(d.group, generic, d.ret));
    } else {
        let _ = writeln!(
            out,
            "  {}({}): {};",
