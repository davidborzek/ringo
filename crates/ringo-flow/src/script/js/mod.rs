...
use crate::engine::{self, Ctx, ScriptHost, TopLevel};
...
pub fn check(path: &Path) -> Result<()> {
    ...
    let err = match host.run_top_level() {
        TopLevel::Single(r) => r.err(),
        TopLevel::Suite { top_error, .. } => top_error,
    };
    match err {
        None => {
