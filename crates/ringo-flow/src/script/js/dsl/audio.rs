...
pub(in crate::script::js) fn audio_tone<'js>(
    cx: Ctx<'js>,
    freq: i64,
) -> rquickjs::Result<Class<'js, AudioSpec>> {
    Class::instance(
        cx,
        AudioSpec {
...
pub(in crate::script::js) fn audio_file<'js>(
    cx: Ctx<'js>,
    path: String,
) -> rquickjs::Result<Class<'js, AudioSpec>> {
    Class::instance(
...
pub(in crate::script::js) async fn verify_audio_connection_async<'js>(
        ...
        .await
    {
        ...
        Err(e) => Err(super::super::convert::throw(
            &cx,
            &format!("verifyAudioConnection task failed: {e}"),
        )),
    }
}
