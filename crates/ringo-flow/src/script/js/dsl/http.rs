...
impl HttpResponse {
    ...
    fn header<'js>(&self, ctx: JsCtx<'js>, name: String) -> JsResult<Value<'js>> {
        into_value(&ctx, self.inner.header(&name).map_or(EngVal::Unit, EngVal::Str))
    }
...
fn http_options<'js>(
    ...
    let body: Value = opts.get("body").map_err(|_| "http options: `body` unreadable".to_string())?;
    let body = if body.is_undefined() || body.is_null() {
        None
    } else if let Some(s) = body.as_string() {
        Some(s.to_string().map_err(|_| "http options: bad `body` string".to_string())?)
