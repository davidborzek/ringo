...
pub(in crate::script::js) fn skip_global(cx: JsCtx<'_>, reason: Opt<String>) -> rquickjs::Result<()> {
...
pub(in crate::script::js) fn setup_global<'js>(#[jsdoc(type = "() => any")] body: Function<'js>) {
    let reg = body.ctx().userdata::<HostState>().expect("host state stored at install").reg.clone();
...
pub(in crate::script::js) fn teardown_global<'js>(#[jsdoc(type = "(ctx: any) => void")] body: Function<'js>) {
    let reg = body.ctx().userdata::<HostState>().expect("host state stored at install").reg.clone();
...
pub(in crate::script::js) fn register_scenario<'a, 'b>(
    ...
    let reg = a.ctx().userdata::<HostState>().expect("host state stored at install").reg.clone();
    ...
    match err {
        JsError::Exception => {
            ...
            match msg.strip_prefix(SKIP_PREFIX) {
                Some(reason) => ScenarioResult::Skipped((!reason.is_empty()).then(|| reason.into())),
