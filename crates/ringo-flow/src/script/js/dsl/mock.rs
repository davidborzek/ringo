...
use rquickjs::{Class, Ctx as JsCtx, Function, IntoJs, JsLifetime, Object, Persistent, Result as JsResult, Value};
...
impl MockServer {
    ...
    #[jsdoc(sig = "respond(method: string, path: string | PathMatch, response: MockResponder): void")]
        ...
        let (method, path_val, resp) = match c.0 {
            Some(resp) => {
                let method = a
                    ...
                    .ok_or_else(|| throw(&ctx, "respond(method, path, response): method must be a string"))?;
                (Some(method), b, resp)
            }
            None => (None, a, b),
        };
...
pub(in crate::script::js) fn regex_global<'js>(cx: JsCtx<'js>, pattern: String) -> rquickjs::Result<Class<'js, PathMatch>> {
    let inner = mock_server::PathMatcher::regex(&pattern)
...
/// Start a mock HTTP server (`mockServer({ port })`); a free port is chosen if none.
#[ringo_flow_macros::ts_global(name = "mockServer")]
pub(in crate::script::js) fn mock_server_global<'js>(
    cx: JsCtx<'js>,
    #[jsdoc(type = "{ port?: number }")] opts: Opt<Object<'js>>,
) -> rquickjs::Result<Class<'js, MockServer>> {
    let (eng, reg) = {
    ...
    Class::instance(cx, MockServer { inner, registry: reg })
}
...
fn mock_response(obj: &Object<'_>) -> MockResponse {
    let headers = obj
        ...
        .unwrap_or_default();
    MockResponse {
        status: obj.get::<_, Option<u16>>("status").ok().flatten().unwrap_or(200),
        content_type: obj.get::<_, Option<String>>("contentType").ok().flatten(),
        body: obj.get::<_, Option<String>>("body").ok().flatten().unwrap_or_default(),
        headers,
    }
}
