...
use rquickjs::{AsyncContext, AsyncRuntime, Ctx as JsCtx, Error as JsError, Function, Module, Persistent, Value};
...
pub struct Registry {
    ...
    pub(super) bridged: Mutex<Vec<(mpsc::Receiver<BridgedRequest>, Persistent<Function<'static>>)>>,
}
...
impl JsHost {
    pub fn new(
        ...
        let entry_name = std::fs::canonicalize(&base_dir)
            ...
            .join(Path::new(&label).file_name().unwrap_or_else(|| std::ffi::OsStr::new("scenario.js")))
            .to_string_lossy()
            .into_owned();
...
impl ScriptHost for JsHost {
    fn run_top_level(&mut self) -> TopLevel {
        ...
        let top: Result<(), String> = futures_executor::block_on(self.context.async_with(async move |ctx| {
...
impl Resolver for FsResolver {
    fn resolve<'js>(
        ...
        let target = if name.starts_with('.') {
            Path::new(base).parent().unwrap_or_else(|| Path::new(".")).join(name)
        } else {
            PathBuf::from(name)
        };
